...
/*
	...
	getCSSClasses : function(editor_id, doc) {
		...
		// Is cached, use that
		if (instance && instance.cssClasses.length > 0)
						...
						if (selectorText) {
							...
							for (var c=0; c<rules.length; c++) {
								...
								if (rule.indexOf('.') != -1) {
									...
									for (var p=0; p<instance.cssClasses.length && addClass; p++) {
										if (instance.cssClasses[p] == cssClass)
											addClass = false;
									}

									if (addClass)
										instance.cssClasses[instance.cssClasses.length] = cssClass;
								}
							}
						}
					}
...
//		return this.contentDocument ? this.contentDocument : this.contentWindow.document; // Removed due to IE 5.5 ?
	...
	_onAdd : function(replace_element, form_element_name, target_document) {
		...
		if (dynamicIFrame) {
			var html = tinyMCE.getParam('doctype') + '<html><head xmlns="http://www.w3.org/1999/xhtml"><base href="' + tinyMCE.settings['base_href'] + '" /><title>blank_page</title><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"></head><body class="mceContentBody cms"></body></html>';
